...
def solve():
    ...
    for i in range(n, 0, -1):
        temp1 = '-'.join(chr(i) for i in range(97 + n - i, 97 + n))
        io.debug(temp1)
        temp2 = '-'.join(chr(i) for i in range(97 + n - i + 1, 97 + n))[::-1] + '-'
        io.debug(temp2)
        l.append(temp2 + temp1)
    temp = []
    ...
    for i in range(len(l)):
        l[i] = (length - len(l[i]))//2 * "-" + l[i] + (length - len(l[i]))//2 * "-"
    for i in l:
        io.write(i)
