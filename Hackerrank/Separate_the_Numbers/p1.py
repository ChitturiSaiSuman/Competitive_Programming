...
def found(s, num):
    ...
    i = 1
    n = len(s)
    while i < n and int(start) != num and int(start) < num and start[0] != "0":
        start += s[i]
        i += 1
    if int(start) == num:
        if i < n:
            return found(s[i:], num+1)
        return True
    return False
...
def solve():
    ...
    for i in range(n):
        start += s[i]
        if found(s[i+1:], int(start)+1) and i+1 != n:
            io.write("YES",int(start))
            return
    io.write("NO")
def main():
    io = IO()
