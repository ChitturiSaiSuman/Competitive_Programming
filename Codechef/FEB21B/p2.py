...
def solve():
    ...
    weight = io.List()
    w = [i for i in weight]
    indices = {i:w.index(i) for i in range(1,n+1)}
    l = io.List()
    costs = {w[i]: l[i] for i in range(n)}
    weight = [[i] for i in weight] + [[] for i in range(100)]
    for i in range(n):
        for j in range(l[i] + i, len(weight), l[i]):
            weight[j].append(w[i])
    ans = 0
    ind = 0
    val = 1
    while ind < len(weight):
        if val in weight[ind]:
            ans += (ind - indices[val])/costs[val]
            val += 1
            ind += 1
        else:
            ind += 1
