...
def solve(g):
    ...
    q = io.Int()
    for i in range(q):
        start, value = io.Tuple()
        io.write(g.solve(start, value))
...
class Graph:
    def __init__(self, V, parent):
        ...
        self.count = Counter(parent)
        for i in range(V-1):
            self.graph[parent[i]].append(i + 2)
        # print(self.count, self.graph)
    
    def solve(self, start, value):
        # print(start, value)
        if start not in self.count:
            return 0
        elif value % self.count[start] != 0:
            return value
        else:
            val = 0
            for child in self.graph[start]:
                val += self.solve(child, value // self.count[start])
            return val
