...
void preCompute()
{
	...
	arr[0] = 2;
	maxN = 1;
	for(int i=3;i<size and power(2,i)<limit;i+=2)
		if(isprime[i])
			arr[maxN++] = power(2,i-1);
	for(int i=3;i<size;i+=2)
	{
		if(isprime[i])
		{
			arr[maxN++] = i;
			for(int j=3;j<size and power(i,j-1)<limit;j+=2)
				if(isprime[j])
					arr[maxN++] = power(i,j-1);
		}
	}
	qsort(arr,maxN,sizeof(ll),compare);
	FOR(i,20)
		printf("%lld ",arr[i]);
}
...
{
	int ind1 = binarySearch(a);
	int ind2 = binarySearch(b);
	println(ind1);
	println(ind2);
	print(ind2-ind1);
}
