...
void solve() {
	...
	int x1 = INT_MAX, y1 = INT_MAX, x2 = INT_MAX, y2 = INT_MIN, x3 = INT_MIN, y3 = INT_MIN, x4 = INT_MIN, y4 = INT_MAX;
	FOR(i, m) {
		FOR(j, n) {
			if(s[i][j] == '1') {
				x1 = min(i, x1);
	...
	if(y1 != y4 or y2 != y3 or x1 != x2 or x3 != x4)
		printsln(YES);
	else {
		if(count == (abs(y2-y1)+1) * (abs(x1-x3)+1))
			printsln(YES);
		else
			printsln(NO);
	}
