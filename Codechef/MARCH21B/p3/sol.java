...
class Main {
    ...
    public static long select(int toBeSelected, int eggsAvailable, int barsAvailable, long omelettePrice, long milkshakePrice) {

        eggsAvailable -= (eggsAvailable&1);
        barsAvailable -= (barsAvailable%3);

        int maxNumOfMilkShakes = barsAvailable/3;
        int maxNumOfOmelettes = eggsAvailable/2;

        // io.debug(maxNumOfMilkShakes, maxNumOfOmelettes, toBeSelected, omelettePrice, milkshakePrice);

        if(maxNumOfMilkShakes + maxNumOfOmelettes < toBeSelected)
            return -1;

        if(omelettePrice == milkshakePrice) {
            if(maxNumOfMilkShakes + maxNumOfOmelettes >= toBeSelected)
                return (toBeSelected * omelettePrice);
            return -1;
        }

        else if(omelettePrice < milkshakePrice) {
            if(maxNumOfOmelettes >= toBeSelected)
                return (toBeSelected * omelettePrice);
            else {
                return (maxNumOfOmelettes * omelettePrice + (toBeSelected - maxNumOfOmelettes) * milkshakePrice);
            }
        }
        
        else {
            if(maxNumOfMilkShakes >= toBeSelected) {
                return toBeSelected * milkshakePrice;
            }
            else {
                return (maxNumOfMilkShakes * milkshakePrice + (toBeSelected - maxNumOfMilkShakes) * omelettePrice);
            }
        }
    }

    public static void solve() {
        long n = io.nextLong(); // number of items required
        long e = io.nextLong(); // number of eggs left in the canteen
        long h = io.nextLong(); // number of chocolate bars left in the canteen
        long a = io.nextLong(); // price of omelette
        long b = io.nextLong(); // price of chocolate milkshake
        long c = io.nextLong(); // price of chocolate cake
        // 2 eggs to make omelette
        // 3 Chocolate bars for chocolate milkshake
        // 1 egg and 1 chocolate bar for chocolate cake
        
        // Base Case: if number of ingredients are not sufficient for n orders, print -1.

        boolean sufficient = false;
        long price = Long.MAX_VALUE;

        int limit = ((int)(Math.min(Math.min(e, h), n)));

        // i represents number of chocolate cakes.
        for(int i = 0; i <= limit; i++) {
            int otherThanCakes = ((int)(n - i));
            int eggsAvailable = ((int)(e - i));
            int barsAvailable = ((int)(h - i));
            long otherPrice = select(otherThanCakes, eggsAvailable, barsAvailable, a, b);
            // io.debug(otherPrice, i * c, otherThanCakes, eggsAvailable, barsAvailable);
            if(i == n) {
                price = Math.min(price, i * c);
                sufficient = true;
            }
            else if(otherPrice == -1)
                continue;
            else {
                price = Math.min(price, otherPrice + i * c);
                sufficient = true;
            }
        }
