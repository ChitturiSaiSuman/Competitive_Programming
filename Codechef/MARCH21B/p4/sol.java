...
class Main {
    ...
    public static boolean isFeasible(long a[], int i, int j) {
        ...
        for(long k = i + 1; k < j; k++) {
            if(y1 * (x2 - x1) + k * y2 - k * y1 - x1 * y2 + x1 * y1 < a[((int)(k))] * (x2- x1))
                return false;
        }
        return true;
    }
    ...
    public static void solve() {
        ...
        int ind1 = 0, ind2 = 0;
        for(int i = 0; i < n - 2; i++) {
            long x_position1 = i + 1;
            long x_position2 = i + 1;
            long max_height = a[i + 1];
            for(int j = i + 2; j < n; j++) {
                if(a[j-1] > max_height) {
                    x_position1 = (j - 1);
                    x_position2 = (j - 1);
                    max_height = a[j-1];
                    // io.debug(i, j, max_height);
                }
                else if(a[j-1] == max_height) {
                    x_position2 = (j - 1);
                }
                long x1 = i;
                long x2 = j;
                ...
                long y2 = a[j];
                long k = x_position1;
                boolean flag1 = ((k * (y2 - y1) + x2 * y1 - x1 * y2) >= (a[((int)(k))] * (x2 - x1)));
                k = x_position2;
                boolean flag2 = ((k * (y2 - y1) + x2 * y1 - x1 * y2) >= (a[((int)(k))] * (x2 - x1)));
                if(flag1 && flag2) {
                    if(j - i > ans) {
                        ans = (j - i);
                        ind1 = i;
