...
ll getRecursive(ll *a, int n, int i1, int i2)
{
	printf("i1,i2 = %d,%d\n",i1,i2);
	if(i2==i1)
		return a[i1];
	...
	for(int i=i1+1;i<i2;i++)
		ans = ans + getRecursive(a,n,i1,i) + getRecursive(a,n,i+1,i2);
	return ans;
}
ll solve(ll *a, int n)
{
	return getRecursive(a,n,0,n-1);
}
...
int main()
{
	...
	For(test,t)
	{
		...
		FOR(i,n)
			scanf("%lld",&a[i]);
		ll cumulative = solve(a,n);
		ll facto = getFactorial(n-1);
		println(cumulative);
		println(facto);
		newline;
		double num = cumulative;
		double den = facto;
